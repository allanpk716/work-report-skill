...
Only task, meeting, reminder, personal, and backlog types support this action.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if cf != nil && !models.IsActionableType(cf.Type) {
			return writeJSONLError("type_not_cancellable", fmt.Sprintf("%s records cannot be cancelled: they are factual records of work already done. Only task, meeting, reminder, personal, and backlog types support the cancel action.", cf.Type))
		}
